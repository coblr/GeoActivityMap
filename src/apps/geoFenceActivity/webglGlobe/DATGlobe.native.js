...
const earthMap = require('./assets/world.jpg');

module.exports = DATGlobe;
...
function DATGlobe(container, opts) {
  ...
  const target = { x: 3.1, y: .65 };
  const targetOnDown = { x: 0, y: 0 };

  let distance = 100000;
  let distanceTarget = 600;
  const padding = 40;
  ...
  function init() {
    ...
    uniforms['texture'].value = new THREE.TextureLoader().load(earthMap);
